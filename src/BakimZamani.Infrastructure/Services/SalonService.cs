...
public class SalonService : ISalonService
{
    ...
    public async Task<ApiResponse<SalonDetailResponse>> GetSalonByIdAsync(string salonId)
    {
        ...
        if (salon == null)
        {
            return ApiResponse<SalonDetailResponse>.Fail("Salon bulunamadÄ±.");
        }
    ...
    public async Task<ApiResponse<SalonDetailResponse>> CreateSalonAsync(string ownerId, CreateSalonRequest request)
    {
        ...
        var ownerName = owner?.FullName ?? "Bilinmiyor";

        // Send notification to all admin users (panel notification) + email to protected admins
        try
        {
            ...
            foreach (var adminId in adminUsers)
            {
                ...
                {
                    UserId = adminId,
                    Title = "Yeni Salon BaÅŸvurusu",
                    Message = $"{salon.Name} ({salon.City}) salonu {ownerName} tarafÄ±ndan oluÅŸturuldu.",
                    Type = NotificationType.System,
                    RelatedEntityType = "Salon",
                    RelatedEntityId = salon.Id,
                };
                _context.Notifications.Add(notification);
            }
            await _context.SaveChangesAsync();

            // Send email to protected admins (fire-and-forget, errors are logged internally)
            _ = _emailService.SendNewSalonNotificationAsync(salon.Name, ownerName, salon.City);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to send new salon notifications for {SalonName}", salon.Name);
        }
    ...
    public async Task<ApiResponse<SalonDetailResponse>> UpdateSalonAsync(string salonId, string ownerId, UpdateSalonRequest request)
    {
        ...
        {
            return ApiResponse<SalonDetailResponse>.Fail("Bu iÅŸlem iÃ§in yetkiniz yok.");
        }
    ...
    public async Task<ApiResponse<StaffResponse>> AddStaffAsync(string salonId, string ownerId, CreateStaffRequest request)
    {
        ...
        {
            return ApiResponse<StaffResponse>.Fail("Salon bulunamadÄ± veya yetkiniz yok.");
        }
    ...
    public async Task<ApiResponse<StaffResponse>> UpdateStaffAsync(string salonId, string staffId, string ownerId, CreateStaffRequest request)
    {
        ...
        {
            return ApiResponse<StaffResponse>.Fail("Personel bulunamadÄ±.");
        }
        ...
        var response = _mapper.Map<StaffResponse>(staff);
        return ApiResponse<StaffResponse>.Ok(response, "Personel gÃ¼ncellendi.");
    }
    ...
    public async Task<ApiResponse> RemoveStaffAsync(string salonId, string staffId, string ownerId)
    {
        ...
        if (salon == null || salon.OwnerId != ownerId)
        {
            return ApiResponse.Fail("Salon bulunamadÄ± veya yetkiniz yok.");
        }

        var staff = await _staffRepository.GetByIdAsync(staffId);
        if (staff == null || staff.SalonId != salonId)
        {
            return ApiResponse.Fail("Personel bulunamadÄ±.");
        }

        _staffRepository.Delete(staff);
        await _unitOfWork.SaveChangesAsync();

        return ApiResponse.Ok("Personel kaldÄ±rÄ±ldÄ±.");
    }
    ...
    public async Task<ApiResponse<ServiceResponse>> AddServiceAsync(string salonId, string ownerId, CreateServiceRequest request)
    {
        ...
        if (salon == null || salon.OwnerId != ownerId)
        {
            return ApiResponse<ServiceResponse>.Fail("Salon bulunamadÄ± veya yetkiniz yok.");
        }
    ...
    public async Task<ApiResponse<ServiceResponse>> UpdateServiceAsync(string salonId, string serviceId, string ownerId, CreateServiceRequest request)
    {
        ...
        {
            return ApiResponse<ServiceResponse>.Fail("Hizmet bulunamadÄ±.");
        }
        ...
        var response = _mapper.Map<ServiceResponse>(service);
        return ApiResponse<ServiceResponse>.Ok(response, "Hizmet gÃ¼ncellendi.");
    }
    ...
    public async Task<ApiResponse> RemoveServiceAsync(string salonId, string serviceId, string ownerId)
    {
        ...
        if (service == null || service.SalonId != salonId)
        {
            return ApiResponse.Fail("Hizmet bulunamadÄ±.");
        }

        _serviceRepository.Delete(service);
        await _unitOfWork.SaveChangesAsync();

        return ApiResponse.Ok("Hizmet kaldÄ±rÄ±ldÄ±.");
    }
    ...
    public async Task<ApiResponse<List<WorkingHoursResponse>>> GetWorkingHoursAsync(string salonId)
    {
        ...
        if (salon == null)
        {
            return ApiResponse<List<WorkingHoursResponse>>.Fail("Salon bulunamadÄ±.");
        }
    ...
    public async Task<ApiResponse<List<WorkingHoursResponse>>> UpdateWorkingHoursAsync(string salonId, string ownerId, UpdateWorkingHoursRequest request)
    {
        ...
        if (salon == null || salon.OwnerId != ownerId)
        {
            return ApiResponse<List<WorkingHoursResponse>>.Fail("Salon bulunamadÄ± veya yetkiniz yok.");
        }
        ...
        return ApiResponse<List<WorkingHoursResponse>>.Ok(response, "Ã‡alÄ±ÅŸma saatleri gÃ¼ncellendi.");
    }
